...
def smna(net_list):
    ...
    def indep_source(line_df, line_nu):
        tk = content[line_nu].split()
        df.loc[line_df,'element'] = tk[0]
        df.loc[line_df,'p node'] = parse_nodes(tk[1])
        df.loc[line_df,'n node'] = parse_nodes(tk[2])
        if tk[3] == 'ac':
            df.loc[line_df,'value'] = parse_value_or_expression(tk[4])
        else:
            df.loc[line_df,'value'] = parse_value_or_expression(tk[3])

    # loads passive elements into branch structure
    def rlc_element(line_df, line_nu):
        tk = content[line_nu].split()
        df.loc[line_df,'element'] = tk[0]
        df.loc[line_df,'p node'] = parse_nodes(tk[1])
        df.loc[line_df,'n node'] = parse_nodes(tk[2])
    ...
    # O - Op Amps
    def opamp_sub_network(line_df, line_nu):
        tk = content[line_nu].split()
        ...
        df.loc[line_df,'element'] = this_element
        
        if bOpampInvertInput:
            df.loc[line_df,'p node'] = parse_nodes(tk[2])
            df.loc[line_df,'n node'] = parse_nodes(tk[1])
        else:
            df.loc[line_df,'p node'] = parse_nodes(tk[1])
            df.loc[line_df,'n node'] = parse_nodes(tk[2])
                
        df.loc[line_df,'Vout'] = parse_nodes(tk[3])
        ...
        if opamp_model == 'OA_integrador':
            ...
            df.loc[line_df,'value'] = gbw/s
            
        elif opamp_model == 'OA_1polo':
            # one-pole opamp model
            df.loc[line_df,'value'] = gbw/(s+gbw/aol)
            
        else:
            # modelo ideal de opamp
            aop = sp.symbols('aop', Real = True)
            df.loc[line_df,'value'] = aop
                

    # G - VCCS
    def vccs_sub_network(line_df, line_nu):
        tk = content[line_nu].split()
        df.loc[line_df,'element'] = tk[0]
        df.loc[line_df,'p node'] = parse_nodes(tk[1])
        df.loc[line_df,'n node'] = parse_nodes(tk[2])
        df.loc[line_df,'cp node'] = parse_nodes(tk[3])
        df.loc[line_df,'cn node'] = parse_nodes(tk[4])
        df.loc[line_df,'value'] = parse_value_or_expression(tk[5])

    # E - VCVS
    # in sympy E is the number 2.718, replacing E with Ea otherwise, sp.sympify() errors out
    def vcvs_sub_network(line_df, line_nu):
        tk = content[line_nu].split()
        df.loc[line_df,'element'] = tk[0].replace('E', 'Ea')
        df.loc[line_df,'p node'] = parse_nodes(tk[1])
        df.loc[line_df,'n node'] = parse_nodes(tk[2])
        df.loc[line_df,'cp node'] = parse_nodes(tk[3])
        df.loc[line_df,'cn node'] = parse_nodes(tk[4])
    ...
    # F - CCCS
    def cccs_sub_network(line_df, line_nu):
        tk = content[line_nu].split()
        df.loc[line_df,'element'] = tk[0]
        df.loc[line_df,'p node'] = parse_nodes(tk[1])
        df.loc[line_df,'n node'] = parse_nodes(tk[2])
        df.loc[line_df,'Vname'] = tk[3].capitalize()
        df.loc[line_df,'value'] = parse_value_or_expression(tk[4])

    # H - CCVS
    def ccvs_sub_network(line_df, line_nu):
        tk = content[line_nu].split()
        df.loc[line_df,'element'] = tk[0]
        df.loc[line_df,'p node'] = parse_nodes(tk[1])
        df.loc[line_df,'n node'] = parse_nodes(tk[2])
    ...
    # K - Coupled inductors
    def cpld_ind_sub_network(line_df, line_nu):
        tk = content[line_nu].split()
        df.loc[line_df,'element'] = tk[0]
        df.loc[line_df,'Lname1'] = tk[1].capitalize()
        df.loc[line_df,'Lname2'] = tk[2].capitalize()
        df.loc[line_df,'value'] = parse_value_or_expression(tk[3])
    ...
    for i in range(cant_netlist_valid):  # process each row in the data frame
        ...
        x = df.loc[i,'element'][0]   #get 1st letter of element name
        if x == 'R':
            g = 1/sp.sympify(df.loc[i,'element'])
        if x == 'C':
            g = s*sp.sympify(df.loc[i,'element'])
        if x == 'G':   #vccs type element
            g = sp.sympify(df.loc[i,'element'].lower())  # use a sp.Symbol for gain value
    ...
    for i in range(cant_netlist_valid):
        ...
        if x == 'E':   # vcvs type, ik column is altered at n1 and n2, cn1 & cn2 get value
            if i_unk > 1:  #is B greater than 1 by n?, E
                ...
                if cn1 != 0:
                    C[sn,cn1-1] = -sp.sympify(df.loc[i,'element'].lower())
                if cn2 != 0:
                    C[sn,cn2-1] = sp.sympify(df.loc[i,'element'].lower())
            else:
                if n1 != 0:
                    C[n1-1] = 1
                if n2 != 0:
                ...
                if vn1 != 0:
                    C[vn1-1] = -sp.sympify(df.loc[i,'element'].lower())
                if vn2 != 0:
                    C[vn2-1] = sp.sympify(df.loc[i,'element'].lower())
            sn += 1   #increment source count
    ...
    for i in range(cant_netlist_valid):
        ...
        if x == 'L':
            if i_unk > 1:  #is D greater than 1 by 1?
                D[sn,sn] += -s*sp.sympify(df.loc[i,'element'])
            else:
                D[sn] += -s*sp.sympify(df.loc[i,'element'])
            sn += 1   #increment source count
        ...
        if x == 'H':  # H: ccvs
            ...
            # then stamp the matrix
            vn1, vn2, df2_index = find_vname(df.loc[i,'Vname'])
            D[sn,df2_index] += -sp.sympify(df.loc[i,'element'].lower())
            sn += 1   #increment source count
        ...
        if x == 'K':  # K: coupled inductors, KXX LYY LZZ value
            ...
            this_M = 'M{:s}'.format(df.loc[i,'element'].lower()[1:])
            ...
            dic_params.update({this_M : df.loc[i,'value'] * sp.sympify('sqrt({:s}*{:s})'.format(df.loc[i,'Lname1'], df.loc[i,'Lname2'] ))} )

            # D[ind1_index,ind2_index] += -s*sp.sympify('{:s}*sqrt({:s}*{:s})'.format(df.loc[i,'element'], df.loc[i,'Lname1'], df.loc[i,'Lname2'] ))  # -s*k*sqrt(L1.L2)
            # D[ind2_index,ind1_index] += -s*sp.sympify('{:s}*sqrt({:s}*{:s})'.format(df.loc[i,'element'], df.loc[i,'Lname1'], df.loc[i,'Lname2'] ))  # -s*k*sqrt(L1.L2)
    ...
    for i in range(len(df2)):
        ...
        J[i] = sp.sympify('I_{:s}'.format(df2.loc[i,'element']))
    ...
    for i in range(cant_netlist_valid):
        ...
        x = df.loc[i,'element'][0]   #get 1st letter of element name
        if x == 'V':
            Ev[sn] = sp.sympify(df.loc[i,'element'])
            sn += 1
