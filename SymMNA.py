...
def smna(net_list):
    ...
    for i in range(line_cnt):
        ...
        #     pass
            ...
            if not match:
                raise Exception("branch {:d} not formatted correctly, {:s} ".format(i,content[i]),
                "Se esperaba una línea estilo: XU1 N001 N002 N003 opamp Aol=100K GBW=10Meg")
    ...
    def indep_source(line_nu):
        ...
        if tk[3] == 'ac':
            df.loc[line_nu,'value'] = parse_value_or_expression(tk[4])
        else:
            df.loc[line_nu,'value'] = parse_value_or_expression(tk[3])
    ...
    def vccs_sub_network(line_nu):
        ...
        df.loc[line_nu,'p node'] = parse_nodes(tk[1])
        df.loc[line_nu,'n node'] = parse_nodes(tk[2])
        df.loc[line_nu,'cp node'] = parse_nodes(tk[3])
        df.loc[line_nu,'cn node'] = parse_nodes(tk[4])
        df.loc[line_nu,'value'] = parse_value_or_expression(tk[5])
    ...
    for i in range(cant_netlist_valid):
        ...
        if x == 'O' or x == 'X':  # op amp type, output connection of the opamp goes in the B matrix
            B[n_vout-1,sn] = 1
            sn += 1   # increment source count
    ...
    for i in range(cant_netlist_valid):
        ...
        x = df.loc[i,'element'][0]   #get 1st letter of element name
        if (x == 'V') or (x == 'O') or (x == 'X') or (x == 'E'):  # need to count V, E & O types
            sn += 1   #increment source count
        ...
        if x == 'K':  # K: coupled inductors, KXX LYY LZZ value
            ...
            D[ind1_index,ind2_index] += -s*sp.sympify('M{:s}'.format(df.loc[i,'element'].lower()[1:]))  # s*Mxx
            D[ind2_index,ind1_index] += -s*sp.sympify('M{:s}'.format(df.loc[i,'element'].lower()[1:]))  # -s*Mxx
    ...
    for i in range(num_nodes):
        ...
        V[i] = sp.Symbol( 'v_{:s}'.format(node_names[i]) )
...
def smna_orig(net_list):
    ...
    for i in range(cant_netlist_valid):  # process each row in the data frame
        ...
        if x == 'C':
            g = s*sp.sympify(df.loc[i,'element'])
        if x == 'G':   #vccs type element
            g = sp.sympify(df.loc[i,'element'].lower())  # use a symbol for gain value
