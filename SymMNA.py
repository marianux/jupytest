...
def smna(net_list):
    ...
    def opamp_sub_network(line_nu):
        ...
        df.loc[line_nu,'element'] = this_element
        
        if bOpampInvertInput:
            df.loc[line_nu,'p node'] = parse_nodes(tk[2])
            df.loc[line_nu,'n node'] = parse_nodes(tk[1])
        else:
            df.loc[line_nu,'p node'] = parse_nodes(tk[1])
            df.loc[line_nu,'n node'] = parse_nodes(tk[2])
    ...
    for i in range(line_cnt):
        ...
        if (x == 'R') or (x == 'L') or (x == 'C'):
            ...
            # parámetro de la simulación (en caso que haya símbolos)
            if tkn[0] == '.param':
